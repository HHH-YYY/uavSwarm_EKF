...
P=diag([0.25,0.25,0.25,0.03,0.03,0.03]);%Initial covariance matrix 初始协方差矩阵 
...
for i=1:M
    ...
    P=diag([0.25,0.25,0.25,0.03,0.03,0.03]);
    Qk=0.001*eye(3);
    R=10;
    for t=tc+1:T 
        Posavai=[];
...
figure(4);
plot(tc:T,GPSRMSE,'g','linewidth',2);
hold on;
plot(tc:T,IMURMSE,'y','linewidth',2);
plot(tc:T,INSRMSE,'r','linewidth',2);
plot(tc:T,PosRMSE,'b','linewidth',2);
xlabel('t/s')
ylabel('RMSE')
title('Position-RMSE')
xlabel('m')
ylabel('m')
zlabel('m')
legend('INSRMSE','IMURMSE','GPSRMSE','EKFRMSE')
grid on;
hold off;
% %% 画出x轴噪声图像
% figure(5);
% xN=[0:15/m:15].';
% plot(xN(1:m),GPSTraj{Dnolost}.TrajPos(:,2)-TrueTraj{Dnolost}.TrajPos(:,2),'y','Linewidth',1,'MarkerSize',2);
% xlim([0,15]);
% ylim([-6,6]);
% hold on;
% plot(xN(1:m),IMUTraj{Dnolost}.TrajPos(:,2)-TrueTraj{Dnolost}.TrajPos(:,2),'b','Linewidth',1,'MarkerSize',2);
% a = X(2,31:201).';
% b = TrueTraj{Dnolost}.TrajPos(110:m,2);
% plot(xN(110:m),a-b,'r','Linewidth',1,'MarkerSize',2);
% 
% xlabel('time/s')
% ylabel('error x/m')
% legend('noised position error of GPS','noised position error of IMU','noised position error of GPSIMU')
% grid on;
% %% 画出y轴噪声图像
% figure(6);
% xN=[0:15/m:15].';
% plot(xN(1:m),GPSTraj{Dnolost}.TrajPos(:,3)-TrueTraj{Dnolost}.TrajPos(:,3),'y','Linewidth',1,'MarkerSize',2);
% xlim([0,15]);
% ylim([-6,6]);
% hold on;
% plot(xN(1:m),IMUTraj{Dnolost}.TrajPos(:,3)-TrueTraj{Dnolost}.TrajPos(:,3),'b','Linewidth',1,'MarkerSize',2);
% a = X(3,31:201).';
% b = TrueTraj{Dnolost}.TrajPos(110:m,3);
% plot(xN(110:m),a-b,'r','Linewidth',1,'MarkerSize',2);
% 
% xlabel('time/s')
% ylabel('error z/m')
...
% figure(7);
% xN=[0:15/m:15].';
% plot(xN(1:m),GPSTraj{Dnolost}.TrajPos(:,1)-TrueTraj{Dnolost}.TrajPos(:,1),'y','Linewidth',1,'MarkerSize',2);
% xlim([0,15]);
% ylim([-6,6]);
% hold on;
% plot(xN(1:m),IMUTraj{Dnolost}.TrajPos(:,1)-TrueTraj{Dnolost}.TrajPos(:,1),'b','Linewidth',1,'MarkerSize',2);
% a = X(1,31:201).';
% b = TrueTraj{Dnolost}.TrajPos(110:m,1);
% plot(xN(110:m),a-b,'r','Linewidth',1,'MarkerSize',2);
% 
% xlabel('time/s')
% ylabel('error z/m')
